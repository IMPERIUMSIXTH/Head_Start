...
class UserPreferences(Base):
    ...
    learning_domains = Column(JSON, default=list)  # ['AI', 'Web Dev', etc.]
    skill_levels = Column(JSON, default=dict)  # {"AI": "beginner", "Python": "intermediate"}
    preferred_content_types = Column(JSON, default=list)  # ['video', 'article', 'paper']
    time_constraints = Column(JSON, default=dict)  # {"max_duration": 30, "sessions_per_week": 3}
    language_preferences = Column(JSON, default=['en'])
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class ContentItem(Base):
    ...
    topics = Column(JSON, default=list)  # Array of topic tags
    language = Column(String(10), default='en')
    content_metadata = Column(JSON, default=dict)  # Source-specific metadata
    embedding = Column(JSON, default=list)  # OpenAI embedding dimension
...
class Recommendation(Base):
    ...
    explanation_factors = Column(JSON, default=dict)  # Factors that influenced the recommendation
